...
echo "SHA256: $(shasum -a 256 MacIrland-v${VERSION}-macos.zip | cut -d' ' -f1)"
echo ""
echo "To update Homebrew formula, run:"
echo "  sed -i '' 's/sha256 \".*\"/sha256 \"$(shasum -a 256 MacIrland-v${VERSION}-macos.zip | cut -d' ' -f1)\"/' homebrew-macirland/Formula/macirland.rb"
