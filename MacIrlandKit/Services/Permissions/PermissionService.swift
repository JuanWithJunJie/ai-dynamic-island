...
public struct AutomationPermissionService: PermissionProviding {
    ...
    func status(terminalAppsDetected: Bool, observationBlocked: Bool) -> CapabilityStatus {
        ...
        if observationBlocked {
            explanation = "已检测到 Terminal / iTerm 正在运行，但当前无法可靠读取或回写会话内容。请确认系统已允许 MacIrland 通过 Apple Events 访问终端应用，然后点击刷新重试。"
        } else if terminalAppsDetected {
            explanation = "已启用真实终端会话发现与 Claude Code reply bridge：当前会尝试通过 Apple Events 读取 Terminal / iTerm 会话，并向目标 Claude Code 会话发送回复；若系统未授权自动化权限，读取或发送都会失败。"
        } else {
            explanation = "已启用真实终端会话发现与 Claude Code reply bridge：请先在 Terminal 或 iTerm 中启动 Claude Code 会话。"
        }
